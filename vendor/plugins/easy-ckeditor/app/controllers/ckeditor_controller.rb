...
class CkeditorController < ActionController::Base
  ...
  def load_file_from_params
		log "L123 paramsssssssssssssss upload #{params[:upload]}"
  ...
  def log_upload
    ...
    log "CKEDITOR - #{File.expand_path(::Rails.root.to_s)}/public#{UPLOAD_FOLDER}/" + "#{@new_file.original_filename}"
  end
  ...
  def uploaded_file_path
		log "204 CKEDITOR ---- upload directory path: #{upload_directory_path}"
		log "205 CKEDITOR ---- upload new file: #{@new_file}"
  ...
  #
  def check_file(file)
		log "213 enter check file ----------------------> file:  #{file}"
    unless [Tempfile, StringIO].contains file.class
			log "215 +++++++++++++++++++++++++++++++++++ ERROR ++++++++++++++++++++++++++++++++++++++++++"
      @errorNumber = 403
      throw Exception.new
    end
		log "219 sort check file ----------------------> file:  #{file}"
    file
  end
